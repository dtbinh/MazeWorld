...
public class BlindRobotMazeDriver extends Application {
	...
	private void initMazeView() {
		maze = Maze.readFromFile("simple2020.maz");
	...
	private void runSearches() {
		...
		BlindRobotMazeProblem mazeProblem = new BlindRobotMazeProblem(maze, gx,	gy);
...
*/
		...
		List<List<Location>> resultPaths = mazeProblem.findPaths(astarPath);
		for (List<Location> path: resultPaths)
			animationPathList.add(new AnimationPath(mazeView, path));
//		animationPathList.add(new AnimationPath(mazeView, resultPaths.get(0)));
//		animationPathList.add(new AnimationPath(mazeView, resultPaths.get(1)));
	...
	private class AnimationPath {
		private Node piece;
		private List<Location> searchPath;
		...
		public AnimationPath(MazeView mazeView, List<Location> path) {
			searchPath = path;
			searchPath.add(searchPath.get(searchPath.size()-1));
			searchPath.add(searchPath.get(searchPath.size()-1));
			searchPath.add(searchPath.get(searchPath.size()-1));

			Location firstNode = (Location) searchPath.get(0);
			piece = mazeView.addPiece(firstNode.getX(), firstNode.getY());
			((Circle)piece).setFill(mazeView.colors[mazeView.currentColor--]);
		...
		public void doNextMove() {
			...
			if (currentMove < searchPath.size() && animationDone) {
				Location mazeNode = (Location) searchPath
						.get(currentMove);
			...
			else if (currentMove >= searchPath.size() && mazeView.getChildren().size() > (maze.width*maze.height)) {
//				System.out.println(mazeView.getChildren().size());
				mazeView.rmPiece(piece);
			}
